 ...
 */
function getEnemySound(type, soundType) {
...
export function generateAudioCues(player, objects) {
    ...
    for (const enemy of enemies) {
        const distance = enemy.distance ?? computeDistance(player.position.x, player.position.y, enemy.position.x, enemy.position.y);
        const distanceBucket = computeDistanceBucket(distance);
        const direction = computeRelativeDirection(player.position.x, player.position.y, player.angle, enemy.position.x, enemy.position.y);
    ...
    if (enemies.length >= 3) {
        const behindCount = enemies.filter((e) => {
            const dir = computeRelativeDirection(player.position.x, player.position.y, player.angle, e.position.x, e.position.y);
 ...
 */
export function describeAudioCues(player, objects) {
