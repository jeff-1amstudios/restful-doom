...
const ENEMY_THREAT_LEVELS: Record<string, "low" | "medium" | "high" | "extreme"> = {
  "FORMER HUMAN": "low",
...
export function categorizeObject(obj: MapObject): ObjectCategory {
  const type = obj.type.toUpperCase();

  if (type === "PLAYER") return "unknown"; // Skip players
  if (ENEMY_TYPES.has(type)) return "enemy";
  if (WEAPON_TYPES.has(type)) return "weapon";
  if (AMMO_TYPES.has(type)) return "ammo";
  if (HEALTH_TYPES.has(type)) return "health";
  if (ARMOR_TYPES.has(type)) return "armor";
  if (POWERUP_TYPES.has(type)) return "powerup";
  if (KEY_TYPES.has(type)) return "key";
  if (HAZARD_TYPES.has(type)) return "hazard";

  return "decoration";
}

export function isEnemy(obj: MapObject): boolean {
  return ENEMY_TYPES.has(obj.type.toUpperCase());
}

export function isPickup(obj: MapObject): boolean {
  const type = obj.type.toUpperCase();
  return (
    WEAPON_TYPES.has(type) ||
    AMMO_TYPES.has(type) ||
...
export function isAlive(obj: MapObject): boolean {
  // Check if object is alive (has health and is shootable)
  return obj.health > 0 && (obj.flags.MF_SHOOTABLE === true);
}

export function getThreatLevel(obj: MapObject): "low" | "medium" | "high" | "extreme" | undefined {
  const type = obj.type.toUpperCase();
...
export function computeRelativeDirection(
  ...
  if (relativeAngle >= 22.5 && relativeAngle < 67.5) return "ahead-left";
  if (relativeAngle >= 67.5 && relativeAngle < 112.5) return "left";
  if (relativeAngle >= 112.5 && relativeAngle < 157.5) return "behind-left";
  if (relativeAngle >= 157.5 || relativeAngle < -157.5) return "behind";
  if (relativeAngle >= -157.5 && relativeAngle < -112.5) return "behind-right";
  if (relativeAngle >= -112.5 && relativeAngle < -67.5) return "right";
  if (relativeAngle >= -67.5 && relativeAngle < -22.5) return "ahead-right";
 ...
 */
export function computeDistanceBucket(distance: number): DistanceBucket {
  if (distance < 256) return "very close";
  if (distance < 512) return "close";
  if (distance < 1024) return "near";
  if (distance < 2048) return "far";
  return "very far";
}
...
export function createSemanticObject(
  ...
  return {
    ...
    direction: computeRelativeDirection(
      player.position.x,
      player.position.y,
      player.angle,
      obj.position.x,
 ...
 */
export function getFriendlyName(type: string): string {
 ...
 */
export function describeObject(obj: SemanticObject): string {
 ...
 */
export function describeObjectGroup(
...
export function describeEnvironment(
  ...
  // Categorize
  const enemies = semanticObjects.filter(
    (o) => o.category === "enemy" && isAlive(o)
  );
  ...
  const enhancedDoors = doors.map((door) => {
    ...
    const distance = door.distance ?? computeDistance(
      player.position.x,
      player.position.y,
      doorX,
      doorY
  ...
  for (const enemy of enemies) {
    if (enemy.threat) threatCounts[enemy.threat]++;
  }
  ...
  if (healthItems.length > 0) nearbyItems.push(`Health: ${healthItems.join(", ")}`);
  if (armorItems.length > 0) nearbyItems.push(`Armor: ${armorItems.join(", ")}`);
  if (weaponItems.length > 0) nearbyItems.push(`Weapons: ${weaponItems.join(", ")}`);
  if (ammoItems.length > 0) nearbyItems.push(`Ammo: ${ammoItems.join(", ")}`);
  if (keyItems.length > 0) nearbyItems.push(`Keys: ${keyItems.join(", ")}`);
  ...
  if (lockedDoors.length > 0) {
    const lockDescs = lockedDoors.map(
      (d) => `${d.keyRequired} key door ${d.direction} (${d.distanceBucket})`
    );
 ...
 */
export function describeWeapon(player: Player): string {
  ...
  const weaponAmmo: Record<number, keyof PlayerAmmo | null> = {
    0: null,
    1: "Bullets",
    2: "Shells",
    3: "Bullets",
 ...
 */
export function describePlayerStatus(player: Player): string {
  const parts: string[] = [];
  ...
  // Keys
  const keys: string[] = [];
  if (player.keyCards.blue) keys.push("blue");
  if (player.keyCards.red) keys.push("red");
  if (player.keyCards.yellow) keys.push("yellow");
  if (keys.length > 0) {
...
export function generateStatusUpdate(
  ...
  objects: MapObject[],
  doors: Door[],
  world: { episode: number; map: number }
): string {
